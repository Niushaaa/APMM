...
Both run APMM + ind via the same bottom-up sweep; per-design total loss must match."""
import numpy as np
...
def main():
    ...
    for seed in range(8):
        ...
        r = K.run_game(g)
        la, li = r["apmm_total"], r["ind_total"]
        worst = max(worst, abs(sa - la), abs(si - li))
        print(f"seed {seed}: apmm sparse={sa:9.4f} lattice={la:9.4f} | "
