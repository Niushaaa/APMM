...
**Goal.** Show that APMM's lower operator loss is a **genuine design efficiency, not
value extracted from traders' pockets**: at a fixed operating point, the informed trader
gets the **same effective price** (expected payout-per-premium) under APMM and ind.
...
`seeds=100`, `base_seed=12345`. Designs: **APMM and ind**. There is **no settlement**:
...
and, pooled, the **per-trader correlation** (Pearson and Spearman) of `p_eff(APMM)`
...
informed trader's expected price is comparable under both designs — APMM's loss advantage
...
in `|S|` (APMM prices high-order parlays slightly more accurately, leaving the trader a
...
Per-trader `p_eff` correlation (APMM vs ind): all `|S|` Pearson 0.78 / Spearman 0.65;
