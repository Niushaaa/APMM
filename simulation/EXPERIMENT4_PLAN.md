...
actual Kalshi NBA same-game-parlay order flow through APMM, ind, and base, settle at the
**realized** game outcomes, and compare operator net loss. Thesis (as in Exp 1): on real
flow, **APMM's parlay loss ≪ ind's**, with base as the floor (no parlays).
...
- **APMM** = `DesignA`: `Q(Sp)=Σ_{T⊆Sp} residual[T]` — pricing a sub-market reuses the lower
...
- **base** = only **genuine base-market trades** (real base flow). It does **not** take the
  ...
  is the base-markets-only floor; its singletons therefore diverge slightly from APMM/ind's
...
- **total loss** per design (`base_total`, `ind_total`, `apmm_total`);
- **parlay loss** `= total − base_total` — the incremental cost of offering parlays over
  the genuine-base-only floor (the APMM-vs-ind signal; base parlay loss ≡ 0).

---

## 5. Metrics & figures

Per game: `M`, `maxK`, parlay-trade count, per-level loss, and (base / ind / APMM / native)
parlay and total loss. Figures (APMM vs ind; **native is table-only**):
- **loss by parlay order** (`fig_loss_by_order`, headline) — grouped bars of corpus total
  loss at each `|S|=2..8`, ind vs APMM: ind humps at 4–5 legs (~52k), APMM ≈ 0 everywhere;
- **parlay loss vs M** (`fig_parlay_loss_vs_M`, y clipped, off-scale ind noted);
- **loss gap `ind − APMM` vs M** (`fig_loss_gap_vs_M`) — gap>0 ⇒ APMM loses less
  (sign-robust): gap>0 in 64/83 games, corpus gap +174,081;
- (`fig_apmm_vs_ind` scatter kept but de-emphasized — the huge dynamic range makes it hard
  to read).

Output: `results_exp4_kalshi/per_game.json`, `fig_parlay_loss_vs_M.png`,
`fig_apmm_vs_ind.png`, `fig_loss_gap_vs_M.png`, `fig_loss_by_order.png`
...
conjunction under ind (own LMSR) and APMM (marginalize touched residuals, §2), record cash
...
(`exp4_validate.py`, 8 seeds): per-design total loss (base, ind, APMM) matches the dense
...
APMM prices the whole parlay market at **essentially zero incremental cost** (parlay P&L
...
| APMM | −25 | −27 | 3 | 19 | 2 | −2 | 0 |

ind humps at 4–5 legs (per-parlay cost rises with order, but high-order parlay *volume*
falls, so the product peaks mid-order); **APMM ≈ 0 at every order** (|·|≤27). Level 1
(singletons) is identical for ind/APMM and equals the base floor.
...
| APMM | −43 | **18,712** ≈ floor |
| native (order book) | — | (size-weighted, reported separately) |

**APMM's total ≈ the base floor** (18,712 vs 18,755): offering the full parlay book costs
APMM almost nothing, whereas `ind`'s total (192,793) is ~10× the floor. The **native**
