#!/usr/bin/env python3
"""
Experiment 4 -- multi-game (cross-game) Kalshi parlay flow through APMM and ind only.
...
def _QA(rA, Sp):                                     # APMM Q(Sp)=Σ_{touched T⊆Sp} rA[T]
...
def main():
    ...
    res = {"apmm_by_level": {str(k): v for k, v in a_lvl.items()},
           "ind_by_level": {str(k): v for k, v in i_lvl.items()},
           "apmm_total": sum(a_lvl.values()), "ind_total": sum(i_lvl.values()),
    ...
    print(f"\nAPMM total loss = {res['apmm_total']:.1f}   ind total loss = {res['ind_total']:.1f}")
    print("loss by parlay order |S|:")
    for j in sorted(set(a_lvl) | set(i_lvl)):
        print(f"  |S|={j:>2}:  ind={i_lvl.get(j,0.0):12.1f}   apmm={a_lvl.get(j,0.0):10.2f}")
