# APMM evaluation — parlay market-maker simulations

Code and experiments for the evaluation section of the **Automated Parlay Market Maker
(APMM)** paper. We compare three LMSR-based market makers for **same-game parlay**
...
- **APMM** (`DesignA`) — hierarchical, **shared** residuals: `Q^{(S)}_ω = Σ_{∅≠T⊆S} r^{(T)}_{ω|_T}`.
...
1. **Scaling** (`experiment1_loworder.py`) — synthetic low-order informed flow; operator
   net loss vs `M`. Result: `base ≤ APMM ≤ ind` pointwise — APMM tracks the floor while
   `ind` grows steeply, the gap widens with `M`, and APMM stays under a `c·M²+d` bound.
2. **Robustness** (`experiment2_robustness.py`) — fix `M=10`, sweep support sparsity `ρ`,
   belief magnitude `B_θ`, re-quote rate `R=T/M`, and order `k`. APMM is robust; `ind`'s
...
3. **Trader fairness** (`experiment3_trader.py`) — per-trader effective payout-per-premium
   `p_eff`; the paired ratio `q = p_eff^{APMM}/p_eff^{ind} ≈ 1` (±10% equivalence). APMM's
...
4. **Real data** (`experiment4_kalshi.py`) — replay actual Kalshi NBA same-game-parlay
   ...
   outcomes. APMM offers the full parlay book at **≈ zero incremental cost** (total ≈ base
